...
app.layout = html.Div(
    ...
    #dcc.Link('The Ultimate Guide to Covid-19', href='https://pragati2527.github.io/COVID_DASHBOARD'
    #html.H1('COVID-19 Tweets - Sentiments  Analysis & Visualization Dashboard', style={'color':"#CECECE",'text-align':'center','margin-top':'30px',}),
    [   html.Div(className='container-fluid', children=[html.Img(src='https://user-images.githubusercontent.com/52466713/87547077-3305d300-c6c8-11ea-90c9-3fa0d937aa34.jpeg', style={'display':'block','margin-bottom':'100px','padding-top':'50px','margin-left':'auto','margin-right':'auto','width':'100%','max-width':'700px','height':'150px'}),
                                                        html.Marquee(html.H2(html.A("The Ultimate Guide to Covid-19", href='https://pragati2527.github.io/COVID_DASHBOARD', target="_blank",style={'text-decoration':'none', 'color': '#6ab04c',}))),
                                                    #    html.Marquee('Self Assessment Test for COVID-19',href="https://jhmcoronavirusselfchecker.azurewebsites.netSelf Assessment Test for COVID-19" ),
                                                        html.H5('Sentiment Term:', style={'color':app_colors['text'], 'margin-top':0, 'margin-bottom': 0,'display':'none'}),
                                                        dcc.Input(id='sentiment_term', value=currentKeyWordsString, type='text',
                                                                  style={'width': 300,'display':'none', 'color':app_colors['someothercolor'], 'margin-top':0, 'margin-bottom': 0}),
        ...
        html.Div(className='container-fluid', children=[html.H5('Window:', style={'color':app_colors['text'], 'margin-top':0,'display':'none', 'margin-bottom': 0}),
                                                        html.Div(className='row', children =
                                                                            [dcc.Input(id='window', value=str(gvalue), type='text',
                                                                                       style={'width': 50,'display':'none', 'color': app_colors['someothercolor'],'margin-top': 0, 'margin-bottom': 10}),
                                                                                html.Button('Submit', id='buttonWindow'),
                                                                                html.Div(id='output-container-button',children='Enter a value and press submit')
                                                                            ]
                                                                         ),
                                                          ],
                         style={'width':'98%','margin-left':15,'margin-right':15,'display':'none','max-width':50000}
                         ),
      ...
      html.Footer(
        html.Div(
            ...
            children=[
                html.Span(
                    'Designed by Amir Abbas and Pragati Chauhan',
                    className='text-muted'),
                html.H5(),
                html.Img(src='https://user-images.githubusercontent.com/52466713/87547209-67798f00-c6c8-11ea-9379-fc899f29e018.jpg', style={'display':'block','margin-left':'auto','margin-right':'auto','width':'45%','max-width':'300px','height':'100px','margin-bottom':'-40px','margin-top':'-20px'}),
            ]),
        ...
        dcc.Interval(
            ...
            interval=2 * 1000
              ...
              'margin-left':-10,'margin-right':-10, 'height':'1050px'},
)
...
def updatePieChart(sentiment_term):
    ...
    try:
        ...
        df = pd.read_sql("SELECT count(case when Polarity = 0.0 then 1 else null end) as Neutral, count(case when Polarity > 0 then 1 else null end) as Positive, \
        ...
        if len(df)>0:
            values = [round(100*df.Positive.iloc[0]/(df.Positive.iloc[0]+df.Negative.iloc[0]+df.Neutral.iloc[0]),2),
                      round(100*df.Negative.iloc[0]/(df.Positive.iloc[0]+df.Negative.iloc[0]+df.Neutral.iloc[0]),2),
                      round(100*df.Neutral.iloc[0]/(df.Positive.iloc[0]+df.Negative.iloc[0]+df.Neutral.iloc[0]),2)]
            print(values)
        else:
            values = [33, 33, 33]
        colors = ['#007F25', '#800000','yellow']
        ...
        return {"data": [trace], 'layout': go.Layout(
            title='Positive vs Negative vs Neutral (Count)',
            font={'color': app_colors['text']},
            plot_bgcolor=app_colors['plotcolor'],
            paper_bgcolor=app_colors['papercolor'],
            showlegend=True)}
